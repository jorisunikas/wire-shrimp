...
std::string formatTable(const std::string &label,
                        const std::unordered_map<std::string, int> &counts) {
    /* automatically gets column width */
    ...
    const size_t countColWidth =
        std::max(std::to_string(maxCount).size(), std::string("Count").size()) +
        2;
    ...
    std::ostringstream oss;
    oss << separator << "\n";
    oss << std::left << std::setw(keyColWidth) << label << std::right
        << std::setw(countColWidth) << "Count" << "\n";
    oss << separator << "\n";

    for (const auto &[key, count] : counts) {
        oss << std::left << std::setw(keyColWidth) << key << std::right
            << std::setw(countColWidth) << count << "\n";
    }
